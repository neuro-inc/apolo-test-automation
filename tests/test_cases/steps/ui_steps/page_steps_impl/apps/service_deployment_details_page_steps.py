...
class ServiceDeploymentDetailsPageSteps:
    ...
    @async_step("Verify that Service Deployment app Details page displayed")
    async def verify_ui_page_displayed(self) -> None:
        assert await self._pm.service_deployment_details_page.is_loaded(), (
            "Service Deployment app Details page should be displayed!"
        )

    @async_step("Verify that app status is valid")
    async def verify_ui_app_status_is_valid(self, expected_status: str) -> None:
        actual_status = await self._pm.service_deployment_details_page.get_app_status()
        assert actual_status.lower() == expected_status.lower(), (
            f"Expected {expected_status} but got {actual_status}!"
        )

    @async_step("Get Service Deployment app UUID")
    async def ui_get_serv_depl_app_uuid(self) -> str:
        return await self._pm.service_deployment_details_page.get_uuid_value()

    @async_step("Validate Service Deployment app details info")
    ...
    async def verify_ui_app_output_displayed(self) -> None:
        assert await self._pm.service_deployment_details_page.is_output_container_displayed(), (
    ...
    async def verify_ui_app_output_apis(self) -> None:
        api_sections = (
            await self._pm.service_deployment_details_page.parse_api_sections()
        )
    ...
    async def verify_ui_app_output_apis_data_format(self) -> None:
        ...
        await self._data_manager.app_data.load_output_ui_schema("service_deployment")

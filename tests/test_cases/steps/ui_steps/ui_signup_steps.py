...
class UISignupSteps:
    ...
    async def verify_ui_org_settings_popup_displayed(
        ...
        assert await self._page_manager.organization_settings_popup.is_loaded(
    ...
    async def ui_click_people_button(self) -> None:
        await self._page_manager.organization_settings_popup.click_people_button()
    ...
    async def verify_ui_org_people_page_displayed(self) -> None:
        assert await self._page_manager.organization_people_page.is_loaded(), (
    ...
    async def ui_click_invite_people_button(self) -> None:
        await self._page_manager.organization_people_page.click_invite_people_button()
    ...
    async def verify_ui_user_displayed_in_users_list(self, email: str) -> None:
        assert (
            await self._page_manager.organization_people_page.is_org_user_row_displayed(
                email
            )
    ...
    async def verify_ui_valid_user_role_displayed(self, email: str, role: str) -> None:
        user_role = await self._page_manager.organization_people_page.get_org_user_role(
            email
        )
    ...
    async def verify_ui_valid_user_status_displayed(
        ...
        user_status = (
            await self._page_manager.organization_people_page.get_org_user_status(email)
        )
    ...
    async def ui_click_invite_user_button(self, email: str) -> None:
        await self._page_manager.invite_member_popup.click_invite_user_button(
            email=email
        )

    @async_step("Verify that Send invite button disabled")
    async def verify_ui_send_invite_button_disabled(self) -> None:
        assert not await self._page_manager.invite_member_popup.is_send_invite_button_enabled(), (
    ...
    async def verify_ui_send_invite_button_enabled(self) -> None:
        assert (
            await self._page_manager.invite_member_popup.is_send_invite_button_enabled()
        ), "Send invite button should be enabled!"

    @async_step("Click Send invite button")
    async def ui_click_send_invite_button(self) -> None:
        await self._page_manager.invite_member_popup.click_send_invite_button()

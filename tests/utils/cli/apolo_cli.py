...
class ApoloCLI:
    def __init__(self):
        self._binary = "apolo"
        self._manager = CLICommandManager(binary=self._binary)
        self._login_success = False
        self._last_command_output = None
        self._parsed_get_orgs_output = []
    ...
    async def run_job(self, job_name, image, command, wait_for_output_timeout=20):
        ...
        job_id_match = re.search(r"Job ID:\s*(job-[\w-]+)", self._last_command_output)
    ...
    async def get_organizations(self):
        ...
        organizations = []
        lines = self._last_command_output.splitlines()
        header_index = None
    ...
    async def verify_login_output(
        ...
        output = self._last_command_output

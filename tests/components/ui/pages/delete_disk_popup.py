...
class DeleteDiskPopup(BasePage):
    ...
    async def is_loaded(self, **kwargs: Any) -> bool:
        ...
        if not isinstance(disk_name, str):
            raise ValueError("Expected 'disk_name' to be a non-empty string in kwargs")
        return (
            await self._get_delete_disk_title().is_visible()
            and await self._get_delete_disk_message(disk_name).is_visible()
            and await self._get_cancel_btn().is_visible()
            and await self._get_delete_btn().is_visible()
        )

    def _get_delete_disk_title(self) -> BaseElement:
        return BaseElement(self.page, "h2", has_text="Delete Disk")

    def _get_delete_disk_message(self, disk_name: str) -> BaseElement:
        return BaseElement(
            self.page,
    ...
    def _get_delete_btn(self) -> BaseElement:
        return BaseElement(self.page, "button", has_text=re.compile(r"^Delete$"))

    async def click_delete_btn(self) -> None:
        self.log("Click Delete button")
        await self._get_delete_btn().click()

    async def wait_to_disappear(self, disk_name: str) -> None:
        """
        Waits until key elements of the popup disappear (popup is closed).
        """
        self.log("Wait for Delete Secret popup to disappear")

        await self._get_delete_disk_title().locator.wait_for(state="detached")
        await self._get_delete_disk_message(disk_name).locator.wait_for(
            state="detached"
        )

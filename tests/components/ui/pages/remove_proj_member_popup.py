...
class RemoveProjMemberPopup(BasePage):
    ...
    async def is_loaded(self, **kwargs: Any) -> bool:
        ...
        return (
            await self._get_remove_member_title(username).is_visible()
            and await self._get_cancel_btn().is_visible()
            and await self._get_remove_btn().is_visible()
        )

    def _get_remove_member_title(self, username: str) -> BaseElement:
        return BaseElement(self.page, "h2.text-h4", has_text=f"Remove User {username}")

    def _get_remove_user_description(self) -> BaseElement:
        return BaseElement(
            self.page,
            "p",
            has_text="The user will no longer have access to the project",
        )
    ...
    async def wait_to_disappear(self, username: str) -> None:
        ...
        await self._get_remove_member_title(username).locator.wait_for(state="detached")
        await self._get_remove_user_description().locator.wait_for(state="detached")
